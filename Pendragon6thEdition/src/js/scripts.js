...
const sheetSelect = (eventinfo) => {
  ...
  update.sheet_type = newValue === "knight" ? "character" : newValue;
  update.character_type = "knight";
...
const roundFraction = (sum) =>
  sum % 1 >= 0.5 ? Math.ceil(sum) : Math.floor(sum);
...
const calculateTreasure = () => {
  getSectionIDs("treasure-record", (idArray) => {
    let attributes = [];
    idArray.forEach((id) =>
      attributes.push(`repeating_treasure-record_${id}_amount_worth`)
    );
...
const calculateMisfortune = ({ triggerName }) => {
  ...
  getAttrs(attributes, (values) => {
    ...
    const care = helpers.convertIntegerNegative(
      parsedNums[`${repeatingRow}_care`]
    );
...
const calculateExpenses = ({ triggerName }) => {
  ...
  getAttrs(attributes, (values) => {
    ...
    const expenses = helpers.convertIntegerNegative(
      parsedNums[`${repeatingRow}_expenses`]
    );
...
const calculateAnnualTotal = (attr) => {
  getSectionIDs("land-record", (idArray) => {
    let attributes = [];
    idArray.forEach((id) =>
      attributes.push(`repeating_land-record_${id}_${attr}`)
    );
...
const calculateRetinueAnnualTotal = () => {
  getSectionIDs("manor-personnel-retinue", (idArray) => {
    let attributes = [];
    idArray.forEach((id) =>
      attributes.push(`repeating_manor-personnel-retinue_${id}_cost`)
    );
