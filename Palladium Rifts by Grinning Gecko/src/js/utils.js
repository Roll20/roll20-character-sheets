...
async function palladiumAddToTurnTracker(initKey, attacksKey) {
  ...
  const addToTracker = await startRoll(
    `@{opt_whisper}&{template:custom} {{title=@{selected|character_name} added to Turn Tracker}} ${nat20bonus === 20 ? '{{Natural 20!=20 points added to initiative roll!}}' : ''} {{Initiative=[[[[${computedWithNat20}]] &{tracker}]]}}`
  );
