        ...
        _.each(mancerdata, function(page) {
            if(usingTasha) {
                _.each(page.values, function(value, name) {
                    if(name.search("ability") !== -1 && name.search("tasha") !== -1 && (name.length == 26 || name.length == 29) && page.values[`${name}_increase`]) {
                ...
                _.each(proficiencyList, function(prof) {
                    _.each(slide.values, function(value, name) {
                        if(name.split("_")[3] == prof.toLowerCase() && name.split("_")[4] == "choice") {
                            if(value && value != "custom" && value.includes(':')) allProficiencies[prof].push(value.split(":")[1]);
                        }
                    });
                });
    ...
    var handleAbilities = function(data, section, usingTasha) {
        ...
        if(usingTasha) {
            ...
            if(data["data-Ability Score Increase"]) {
                const increase = JSON.parse(data["data-Ability Score Increase"]);
                    ...
                    addRepeatingSection(subsection, "row", function(rowid) {
                        ...
                        if(!json.Hidden) {
                            if(prof === "Skill" && options.section && options.section === "background") {
                                ...
                                const duplicatedSkills = json.Proficiencies.filter(skill => { return allAutoSkills.indexOf(skill) > -1; });
                                json.Proficiencies = json.Proficiencies.filter(skill => { return duplicatedSkills.indexOf(skill) === -1; });
        ...
        _.each(sorted.swaps, function(blob, name) {
            addRepeatingSection(element, "row", function(rowid) {
                ...
                const getSwapable = function() {
                    ...
                    const previousRepeating = JSON.parse(mancerdata['lp-welcome']['values']['previous_repeating']);
                ...
                for(let x=1; x<=choicenum; x++) {
                    addRepeatingSection(rowid + " label", "swap", section + "_feature", function(id) {
                        const swapChoices = swapable.map(entry => { return entry.name.split(':')[1].trim(); });
    ...
    on("mancerchange:abilities", function(eventinfo) {
        ...
        var attribs = globalAttributesByCategory.abilities.concat(optionalAttributes);
        var standardArray = ["8", "10"].concat(optionalArray).concat(["12", "13", "14", "15"]).map((value, index) => { return `${value}~${index+1}`});
    ...
    on("mancerchange:strength mancerchange:dexterity mancerchange:constitution mancerchange:intelligence mancerchange:wisdom mancerchange:charisma mancerchange:honor mancerchange:sanity", function(eventinfo) {
        const optionalAttributes = SheetUtils.toLowerCase(abilityListEnabledOptionals);
        data = getCharmancerData();
        var attribs = globalAttributesByCategory.abilities.concat(optionalAttributes);
    ...
    on("page:l1-equipment", function(eventinfo) {
        ...
        const class_equipment = data && data["l1-equipment"] && data["l1-equipment"].values && data["l1-equipment"].values.equipment_class ? data["l1-equipment"].values.equipment_class : undefined;
        const background_equipment = data && data["l1-equipment"] && data["l1-equipment"].values ? data["l1-class"].values.equipment_background : undefined;
        let update = {};
        let update_attr = {};
        let showList = [];
        let hideList = [];
        let allBlobs = getRelevantBlobs(data, "1", "l1");
        let equipmentType = data["l1-equipment"] && data["l1-equipment"].values && data["l1-equipment"].values.equipment_type ? data["l1-equipment"].values.equipment_type : "";
        ...
        const handleChoices = type => {
            ...
            if (allBlobs.sorted[type]) {
                _.each(allBlobs.sorted[type], blob => {
                    if (blob.Equipment) {
            ...
            _.each(equipment, function(feature, num) {
                if(num === "default") {
                    ...
                    let equipmentTitle = cleanEquipment(feature).replace(/^\w/, char => char.toUpperCase()).replace(/(^\(|\)$)/g, "");
            ...
            if(type == "feature") {
                if(eventinfo.previousValue !== eventinfo.newValue){
                    ...
                    if(eventinfo.newValue.split(":")[0] == "Blob") {
                        if(pagedata.blobs[eventinfo.newValue.split(":")[1]]["Pick Spells"] || pagedata.blobs[eventinfo.newValue.split(":")[1]].Powers) {
            ...
            _.each(sorted, function(spellarray, level) {
                _.each(spellarray, function(spells, section) {
                    addRepeatingSection("spell_holder", "row", "spellrow", function(rowid) {
                        ...
                        var knownspells = [];
                        var toset = {};
                        repupdate[rowid + " label span"] = level == 0 ? titles[section] + " Prime" : titles[section] + " Rank " + level;
                        ...
                        _.each(spells.choices, function(spell) {
                            addRepeatingSection(rowid, "choose", section + "_spell-l" + spell.Level, function(id) {
                                ...
                                getCompendiumQuery(choices, data => {
                                    let options = new Set(data.map(entry => { return entry.name; }).concat(spell["Expanded List"]));
                                    options = Array.from(options).sort().filter(entry => { return (typeof entry === 'string'); });
                                    settings.disable = settings.disable.filter(entry => { return options.indexOf(entry) > -1 });
    ...
    on("page:l1-feat", function(eventinfo) {
        ...
        if(SheetUtils.checkPath(mancerdata, "['l1-race']['data']['subrace']['data-Feats']") || SheetUtils.checkPath(mancerdata, "['l1-race']['data']['race']['data-Feats']")) {
            showList = ["yes_feat"];
            hideList = ["no_feat"];
        } else {
            hideList = ["yes_feat"];
    ...
    on("mancerchange:feat", function(eventinfo) {
        ...
        if(eventinfo.sourceType == "player") {
            ...
            reset['feat_expertise_choice1'] = '';
            reset['feat_expertise_choice2'] = '';
        ...
        getCompendiumPage(eventinfo.newValue, function(p) {
            ...
            setAttrs(reset, function() {
                ...
                if(data["Other Options and Features"]) {
                    const otherFeatures = JSON.parse(data["Other Options and Features"]);
                ...
                if(data["data-Pick Spells"]) {
                    const spells = JSON.parse(data["data-Pick Spells"]);
    ...
    on("page:l1-summary", function() {
            ...
            if((raceSpells + classSpells) == 0) {
                ...
                if(prevClass != currentClass || mancerdata["l1-spells"].values.class_number > classSpells) {
                    ...
                    if(classSpells > 0 && mancerdata["l1-spells"].values.class_number > 0) {
                        set["spells_info"] += '<p class="sheet-warning">Your class spells were reset because your options have changed.</p>';
                    }
                }
            }
        } else if((raceSpells + classSpells) > 0) {
            set["spells_info"] = '<p class="sheet-warning">You haven\'t selected your spells!</p>';
            spellsready = false;
        ...
        deleteCharmancerData([{"l1-spells":toDelete}], function() {
            ...
            if(spellsready) {
                ...
                _.each(choices, function(number, type) {
                    if(number[1] > 0) {
                        set["spells_info"] += '<p class="sheet-warning">You can choose ';
    ...
    on("mancerfinish:l1-mancer", function(eventinfo) {
        ...
        if(data["l1-feat"] && data["l1-feat"].values.feat) {
            ...
            const otherOptionsAndFeatures = Object.keys(data['l1-feat']['values']).filter(entry => { return entry.includes('other_options_and_features'); });
            ...
            const skills = Object.keys(data['l1-feat']['values']).filter(entry => { return entry.includes('feat_skill_choice'); });
            ...
            const expertise = Object.keys(data['l1-feat']['values']).filter(entry => { return entry.includes('feat_expertise_choice'); });
            ...
            const spells = Object.keys(data['l1-feat']['values']).filter(entry => { return entry.includes('feat_spell_choice'); });
            let i = 0;
                    ...
                    if(SheetUtils.checkPath(data, "['l1-feat']['data']['feat']['data-Pick Spells']")) {
                        ...
                        if(pickData['Spellcasting Ability']) {
                            ...
                            if(spellcastingAbility === 'asi') {
                                const asiKey = Object.keys(data['l1-feat']['values']).filter(entry => { return entry.includes('feat_ability_choice') });
                                if(asiKey) {
        ...
        eraseRepeating(eraseSections, function() {
            ...
            setAttrs(silentset, {silent:true}, function() {
                ...
                getAttrs(globalModifiers, modifiers => {
                    const modifiersToggledOn = Object.keys(modifiers).filter(key => { return modifiers[key] === "1"; });
                    modifiersToggledOn.forEach(key => { delete clearset[key]; });
    ...
    on("page:l1-abilities", eventinfo => {
        ...
        const attribsRoll = ["strength","dexterity","constitution","intelligence","wisdom","charisma"].concat(optionalAttributes).map((key, index) => `{{r${index+1}=[[4d6dl1]]}}`).join(' ');
        ...
        setCharmancerText({"sheet-rollstats-button": rollstatsButton});
    });

    on("page:l1-welcome page:l1-race page:l1-class page:l1-abilities page:l1-background page:l1-equipment page:l1-spells page:l1-feat page:l1-bio page:l1-summary", eventInfo => {
        showOptionalAttributeFields();
    });

    on('mancerchange:customize_origin', eventinfo => {
        if(eventinfo.sourceType && eventinfo.sourceType == "player") {
            deleteCharmancerData(["l1-race"], data => {
    ...
    on("mancerchange:feat_skill_choice1 mancerchange:feat_skill_choice2 mancerchange:feat_skill_choice3 mancerchange:feat_skill_choice4", function(eventinfo) {
