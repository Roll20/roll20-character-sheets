...
/*********************************************************************/
 ...
 //Helper function to update the text in the top bar
            ...
            _.each(mancerdata["lp-asi"].repeating, function(repid) {
                if(repid.substr(31) == "asi-row" && mancerdata["lp-asi"].values[repid + "_switch"] != "feat") {
                    ...
                    if(SheetUtils.checkPath(mancerdata, `['lp-asi']['values']['repeating_${rowID}_asi-row_feat']`)) {
                        ...
                        _.each(lcAbilities, function(ability) {
                            const choiceIncrease = (mancerdata['lp-asi']['values'][`repeating_${rowID}_asi-row_feat_ability_choice`] && mancerdata['lp-asi']['values'][`repeating_${rowID}_asi-row_feat_ability_choice`].toLowerCase() == ability) ? 1 : 0;
    ...
    var getFeatAbilityIncrease = function(data, rowID) {
        ...
        var asi = JSON.parse(data['lp-asi']['data'][`repeating_${rowID}_asi-row_feat`]['data-Ability Score Increase']);
        _.each(asi, function(increase, ability) {
    ...
    on("page:lp-levels", function(eventinfo) {
        ...
        if(SheetUtils.checkPath(mancerdata, "['l1-class']['values']['class']") && /\?expansion=[0-9]/g.test(mancerdata['l1-class']['values']['class'])) {
        ...
        update["class1_selector"] = '<span class="sheet-compendium-class-name">' + removeExpansionInfo(baseclass) + '</span>';
    ...
    on("mancerchange:class1_classlevel mancerchange:class2_classlevel mancerchange:class3_classlevel mancerchange:class4_classlevel", function(eventinfo) {
        ...
        update["levels_message"] = `<p>Character will be level ${characterlevel} (${end.map(entry => { return removeExpansionInfo(entry); })})</p>`;
    ...
    var getLpRaceData = function(mancerdata, leveldata) {
        ...
        const characterlevel = SheetUtils.checkPath(mancerdata, '["lp-levels"]["values"]["characterlevel"]') ? Math.max(parseInt(mancerdata["lp-levels"]["values"]["characterlevel"]) - addlevel, 0) : 0;
    ...
    const readPreviousFeats = function(data) {
        ...
        const previousData = JSON.parse(data['lp-welcome']['values']['previous_repeating']);
        if(!SheetUtils.checkPath(previousData, "['traits']")) return;
        let feats = previousData['traits'].filter(trait => { return trait.source === 'Feat'; }).map(trait => { return `${trait.name}`; });
        return feats;
    }
    on("page:lp-asi", function(eventinfo) {
        setCharmancerText({'sheet-spinner-toggle': 'Loading...'});
            ...
            _.each(asis, function(asi) {
                addRepeatingSection("choices", "asi-row", function(rowid) {
                    ...
                    getCompendiumQuery('Category:Feats', data => {
                        ...
                        counter++;
                        if(counter === asis.length) restorePreviousASI(previousState, () => { setCharmancerText({'sheet-spinner-toggle': ''}); });
                    });
                });
            });
        });
    ...
    on("page:lp-spells", function(eventinfo) {
        ...
        if(classes.length > 0) {
            ...
            setCharmancerText(update);
            queries = queries.map(query => { return removeExpansionInfo(query); });
    ...
    const resetASI = function(id) {
        let reset = {};
        ['strength', 'dexterity', 'constitution', 'intelligence', 'wisdom', 'charisma', 'sanity'].
        forEach(ability => {
    ...
    on("mancerchange:repeating_asi-row_feat", eventinfo => {
        ...
        const rowClass = `sheet-repeating_${rowID}_asi-row`;
    ...
    on("page:lp-summary", function() {
        ...
        const noFeatsMsg = '<p>As you exchange your novice notions of the world for experience, a feat may become within reach. Not today, however.</p>';
    ...
    on("mancerfinish:lp-mancer", function(eventinfo) {
        ...
        var getRemovableSwaps = function() {
            ...
            const swaps = Object.keys(data['lp-choices']['values']).filter(entry => { return entry.indexOf('_feature_swap') > -1; });
            let removableSwaps = []
        ...
        const getLPFeats = () => {
            if(!SheetUtils.checkPath(data, "['lp-asi']['values']")) return 0;
                ...
                if(featSelected) {
                    const otherOptionsAndFeatures = Object.keys(data['lp-asi']['values']).filter(entry => { return entry.includes('other_options_and_features'); });
                    ...
                    const skills = Object.keys(data['lp-asi']['values']).filter(entry => { return entry.includes('feat_skill_choice'); });
                    ...
                    const expertise = Object.keys(data['lp-asi']['values']).filter(entry => { return entry.includes('feat_expertise_choice'); });
                    ...
                    const spells = Object.keys(data['lp-asi']['values']).filter(entry => { return entry.includes('feat_spell_choice'); });
                    let i = 0;
                    spells.forEach(spell => {
                        if(data['lp-asi']['values'][spell]) {
                            ...
                            if (SheetUtils.checkPath(data, "['lp-asi']['data']") && data['lp-asi']['data'][`repeating_${rowID}_asi-row_feat`] && data['lp-asi']['data'][`repeating_${rowID}_asi-row_feat`]['data-Pick Spells']) {
                                const pickData = JSON.parse(data['lp-asi']['data'][`repeating_${rowID}_asi-row_feat`]['data-Pick Spells'])[i];
                                if(pickData['Spellcasting Ability']) {
                                    ...
                                    if(spellcastingAbility === 'asi') {
                                        let asiKey = (SheetUtils.checkPath(data, "['lp-asi']['values']") && data['lp-asi']['values'][`repeating_${rowID}_asi-row_feat_ability_choice`]) ? data['lp-asi']['values'][`repeating_${rowID}_asi-row_feat_ability_choice`] : false;
                                        if(asiKey) {
    ...
    on("mancerchange:repeating_row", function(eventinfo) {
        if(eventinfo.sourceAttribute && eventinfo.sourceAttribute.indexOf('feature_swap') > -1) {
            ...
            const flag = `${eventinfo.sourceSection} .sheet-swap-selected`;
    ...
    on("mancerchange:repeating_asi-row_feat_skill_choice1 mancerchange:repeating_asi-row_feat_skill_choice2 mancerchange:repeating_asi-row_feat_skill_choice3 mancerchange:repeating_asi-row_feat_skill_choice4", eventinfo => {
        if(eventinfo.sourceType && eventinfo.sourceType === 'player') {
            getRepeatingSections('choices', repeating => {
                const asi = repeating.list.filter(entry => { return entry.includes('asi-row'); });
