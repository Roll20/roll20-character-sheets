    ...
    if(mancerdata["l1-equipment"] && mancerdata["l1-equipment"].values["equipment_type"]) {
        switch(mancerdata["l1-equipment"].values["equipment_type"]) {
            case "class":
                ...
                break;
            case "cubil":
                set["equipment_info"] = '<p>You opted for cubil instead of starting equipment.</p>';
                break;
        }
        ...
        if(mancerdata["l1-equipment"].values["equipment_type"] == "cubil" && mancerdata["l1-equipment"].values["starting_cubil"] == "0") {
            set["equipment_info"] = '<p class="sheet-warning">You selected starting wealth for your equipment, but you don\'t have any cubil yet!</p>';
        }
...
on("page:lp-spells", function(eventinfo) {
    ...
    if(classes.length > 0) {
        ...
        if(newprimes != 0 || knownprimes != 0) update["spells-summary"] = "<div class=\"level_0\"><div class=\"title\">Primes: </div><div class=\"list\"></div></div>";
        ...
        if(classes.length > 1) {
            ...
            _.each(classes, function(thisclass) {
                if(thisclass.newspells) {
                    update["primeinfo"] += "<p>You can add " + thisclass.newprimes + " " + thisclass.class + " primes.</p>";
                }
            });
        }
        setCharmancerText(update);
    ...
    getRepeatingSections(replist.find((x) => {return x.includes("spellsrow")}), function(repeating) {
        ...
        if(sourcelevel == "0") {
            ...
            if(thispage != "lp-spellchoice") {
                ...
                if(replaced > 0) {
                    update[primesection + " .sheet-spellstext .sheet-total"] += "<br>(" + allowed + " new, " + replaced + " replaced)";
                }
                if(allowedreplace > 0) {
            ...
            _.each(perlevel, function(info, level) {
                if(thispage != "lp-spellchoice") update[primesection + " .sheet-spellstext .sheet-levels"] += "<p>" + info.number + " Level " + level + " spells added.</p>";
...
on("page:lp-summary", function() {
    ...
    if(selectedPowers.length > 0) update["spell_info"] = '<p>You have learnt these spells: '+ selectedPowers.sort().join(", ") +'</p>';
