...
def _match_lengths(
    ...
    if left_len != right_len and left_len != 1 and right_len != 1:
        raise ValueError(f"Cannot match {lname} length {left_len} to {rname} length {right_len}")
...
def repeat_format(
    ...
    *,
    ...
    if (keys is None) != (values is None):
        raise ValueError("Dictionary formatting requires both 'keys' and " "'values' arguments")
    ...
    if replace is not None and by is not None:
        ...
        if keys is not None and values is not None:
            ...
            raise RuntimeError
        return separator.join(
            string % {key.lower(): value.lower(), key.title(): value.title(), key: value}
...
class RollTemplate:
    ...
    def __post_init__(self):
        ...
        if invalids := (self.fields.keys() - self.TEMPLATES[self.name]):
            raise ValueError(
                "rolltemplate '%s' has no key %s. %s" % (self.name, ", ".join(invalids), msg)
            )
