...
const mainAttributeArr = [
  ...
  {name:"strength_monster",attr:"strength_monster"},
  ...
  ***********************/
  ...
  const versionator = async (old_version) => {
    ...
    switch (true) {
        ...
        case old_version < 3.00:            
            ...
            break; 
        default:
            clog("----- In switch default, no update needed. ");            
            await setAttrsAsync({is_new_character: 0});
            new_character = (await getAttrsAsync(["is_new_character"])).is_new_character;
            clog("New character: 0");
  ...
  const update_to_3_00 = async (old_version, upgraded_version) => {
    ...
    clog(`** UPDATING "${charname.character_name}" TO V3.00 FROM ${old_version} **`);
  ...
  */
  ...
  on("clicked:roll_the_dice", () => {
    getAttrs( ["character_name", "character_id", "attribute", "skill", "gear", "current_preset", "whisper_option", "pushable"], async (values) => {
  ...
  const executeRoll = function({trigger,attributes,roll}) {
    ...
    if (roll.pushRollStr) {
      ...
      rollStr = `&{template:mutantyearzero} {{character_id=${attributes['character_id']}}} {{character_name=${attributes['character_name']}}} {{name=${roll.name}}} {{successes=[[0]]}} {{push=[[${roll.pushable}]]}} {{pushroll=[[${roll.pushroll}]]}} ${attributeStr} ${skillStr} ${gearStr}`;
    }    
    ...
    startRoll(rollStr, (results) => {
      ...
      roll.pushRollStr = `&{template:mutantyearzero} {{character_id=${attributes['character_id']}}} {{character_name=${attributes['character_name']}}} {{name=${roll.name}}} {{push=[[${roll.pushable}]]}} {{pushroll=[[${roll.pushroll}]]}} {{successes=[[${successes}]]}} {{failures=[[${failures}]]}} {{gearfails=[[${gearfails}]]}} ${pushAttributeStr} ${pushSkillStr} ${pushGearStr}`;
