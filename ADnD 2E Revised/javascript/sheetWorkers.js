...
function bookInactiveShowToast(books, obj) {
    let activeBooks = Object.entries(books)
        .filter(([book, _]) => BOOK_FIELDS.includes(book))
        .flatMap(arr => arr[1]);
    let bookInactive = !activeBooks.includes(obj['book']);
    if (bookInactive)
...
async function calculateFormula(formulaField, calculatedField, defaultValue) {
...
const updateThac0 = () => calculateFormula('thac0-base', 'thac0-base-calc', 20);
on('change:thac0-base', updateThac0);
...
function setWeaponWithBonus(weaponName, setWeaponFunc, thac0Field, isMonster) {
    ...
    getAttrs(fields, function(values) {
        ...
        let thac0 = isNaN(thac0Int) ? 20 : thac0Int;
