...
The first page also includes a portrait section designed as a slideshow. The first shown is a "rule card" that is by default a welcome message. The second is the player's avatar image that can be assigned via the Character's "Bio and Info" tab. The third is a sticky note for text.
