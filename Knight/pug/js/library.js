...
{   
    let listAttrs = ['version'];
    const attrs = await getAttrsAsync(listAttrs);
    const version = parseInt(attrs["version"], 10)||0;
    
    let list = [];

    if(version < 15)
    {    
        list.push("MAJSheetworker", "MonkMAJ", "MAJ", "PsionMAJ", "OD_MAJ", "PNJAEMAJ", "PNJPFMAJ", "WarriorOD_MAJ", "WarriorOD_MAJ_2", "PJINSMAJ", "MAJV15LDB_1", "MAJV15LDB_2", "MAJV15LDB_3", "DruidLionBF")
        const value = await getAttrsAsync(list);
    ...
    if(version < 19)
    {
        const vXP =  await getAttrsAsync(['expUti', 'expAct', 'gloAct', 'gloUti']);
        const getXPUti = parseInt(vXP["expUti"], 10)||0;
        const getXPAct = parseInt(vXP["expAct"], 10)||0;
        const getGLUti = parseInt(vXP["gloAct"], 10)||0;
        const getGLAct = parseInt(vXP["gloUti"], 10)||0;
        const update = {
            'exp-totale':getXPAct+getXPUti,
            'exp-utilisee':getXPUti,
            'glo-totale':getGLUti+getGLAct,
            'glo-utilisee':getGLUti,
            'version':19,
        }
