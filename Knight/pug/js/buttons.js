...
//CHOIX ARMURE
on(`clicked:choixArmure`, function() {                
    setAttrs({
        popup: 1
    });
});

on(`change:choixArmure`, function(event) {
...
buttonGainEspoir.forEach(button => {
    on(`clicked:${button}`, function() {    
        ...
        getAttrs([name], function(value)
        {
            let active = value[name];
...
//SELECTION ARMURE
on(`clicked:selectionArmure`, function() {                
    setAttrs({
        popup: 0
    });
});
...
on(`change:armureLegende`, function() {                
    setAttrs({
...
buttonCaracBonus.forEach(button => {
    on(`clicked:${button}`, function() {    
        ...
        {
            let bonusCarac = parseInt(value["bonusCarac"], 10)||0;
            let newValue = button.split("_")[1];

            if(bonusCarac == newValue)
                newValue = 0;
...
buttonCapaciteActive.forEach(button => {
    on(`clicked:${button}`, function() {    
        ...
        getAttrs(["capaciteUltimeType"], function(value)
        {
            let capaciteActive = parseInt(value["capaciteUltimeType"], 10)||0;
            let newValue = button.split("_")[1];

            if(capaciteActive == newValue)
                newValue = 0;
...
buttonBerserk.forEach(button => {
    on(`clicked:${button}`, function() {    
        ...
        getAttrs([button], function(value)
        {
            let active = value[button];
            let newValue = "on";
...
buttonNiveauRage.forEach(button => {
    on(`clicked:${button}`, function() {    
        ...
        getAttrs(["berserkNiveaux"], function(value)
        {
            let niveauRage = parseInt(value["berserkNiveaux"], 10)||0;
            let newValue = button.split("_")[1];

            if(niveauRage == newValue)
                newValue = 0;
    ...
    on(`clicked:${button}`, function(eventinfo) {
        ...
        if(source != undefined) {
            if(source.includes("repeating")) {
                let length = source.split("-").length-1;
...
buttonPaladin.forEach(button => {
    on(`clicked:${button}`, function() {                
        getAttrs([button], function(value)
        {
            ...
            if(active == "Activé" || active == "Activer")
                newValue = "";
...
buttonShaman.forEach(button => {
    on(`clicked:${button}`, function() {                
        getAttrs(["shamanAscension"], function(value)
        {
            let active = value[button];
            let newValue = "1";
...
buttonSorcerer.forEach(button => {
    on(`clicked:${button}`, function() {                
        getAttrs(["sorcererMM250PG"], function(value)
        {
            let active = value[button];
            let newValue = "1";
