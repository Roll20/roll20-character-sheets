...
var setDiceFromTotal = (name, numDice, upToFive, value) => {
	...
	fillRepeatingSectionFromData = (sectionName, dataList, autoGenerated) => {
		'use strict';
	...
	getAttrs(['crew_type', 'playbook', 'changed_attributes'], v => {
		...
		case 'crew_type':
			...
			if (crewData.hasOwnProperty(sourceName)) {
				...
				deleteAutogenRows('contact');
				deleteAutogenRows('crewability');
				deleteAutogenRows('upgrade');
				fillRepeatingSectionFromData('contact', crewData[sourceName].contacts, true);
				fillRepeatingSectionFromData('crewability', crewData[sourceName].abilities, true);
				fillRepeatingSectionFromData('upgrade', crewData[sourceName].upgrades, true);
			}
			break;
		case 'playbook':
			...
			if (playbookData.hasOwnProperty(sourceName)) {
				...
				deleteAutogenRows('friend');
				deleteAutogenRows('ability');
				deleteAutogenRows('playbookitem');
				fillRepeatingSectionFromData('playbookitem', playbookData[sourceName].items, true);
				fillRepeatingSectionFromData('friend', playbookData[sourceName].friends, true);
				fillRepeatingSectionFromData('ability', playbookData[sourceName].abilities, true);
			}
		};
...
on('sheet:opened', () => {
	...
	getAttrs(['version'], v => {
		...
		setAttrs({
			version: '1.7',
			character_sheet: 'Blades in the Dark v1.7'
		});
	});
});
</script>
