...
PowerCard.Process = function(msg, player_obj) {
	...
	if (PowerCard.title === undefined) PowerCard.title = msg.who;
	...
	if (CUSTOM_EMOTES && PowerCard.emote !== undefined && PowerCard.charid !== undefined || PowerCard.tokenid !== undefined) {
		...
		if (SHOW_AVATAR) PowerCard.emote = Avatar + '<div style="display: table-cell; width: calc(100%-50px); vertical-align: middle; font-size: 12px; font-style: italic; text-align: center; padding-left: 0.5em;">' + doInlineFormatting(PowerCard.emote) + '</div>';
		else PowerCard.emote = '<div style="display: block; width: 100%; vertical-align: middle; font-size: 12px; font-style: italic; text-align: center;">' + PowerCard.emote + '</div>';
	}
			...
			sendChat("", Display, function(x) {
				...
				if (PowerCard.whisper !== undefined) {
					if (PowerCard.emote !== undefined) {
						...
						} else sendChat("", "/emas " + PowerCard.emote);
					}
...
function doInlineFormatting(content, ALLOW_URLS, ALLOW_HIDDEN_URLS) {
	...
	var f;
	// LINE BREAK
	if (content.indexOf("^^") !== -1) content = content.replace(/\^\^/g, "<br>");
	// INDENT FIRST LINE
	...
	while (content.indexOf("$$") !== -1) {
		...
		if (f % 2 == 1) {
			...
			if (color !== null) color = color.shift().replace("$$", "").replace("|", "");
...
function doTargetInfo(content, TargetList) {
	...
	// TOKEN CHARMS
	if (content.indexOf("%%token_name%%") !== -1) content = content.replace("%%token_name%%", Token.get("name"));
	if (content.indexOf("%%bar1%%") !== -1) content = content.replace("%%bar1%%", Token.get("bar1_value"));
	if (content.indexOf("%%bar1_max%%") !== -1) content = content.replace("%%bar1_max%%", Token.get("bar1_max"));
	if (content.indexOf("%%bar2%%") !== -1) content = content.replace("%%bar2%%", Token.get("bar2_value"));
	if (content.indexOf("%%bar2_max%%") !== -1) content = content.replace("%%bar2_max%%", Token.get("bar2_max"));
	if (content.indexOf("%%bar3%%") !== -1) content = content.replace("%%bar3%%", Token.get("bar3_value"));
	if (content.indexOf("%%bar3_max%%") !== -1) content = content.replace("%%bar3_max%%", Token.get("bar3_max"));
...
function processRoll(roll, critRoll, failRoll, highRoll, lowRoll, noHighlight) {
	if (roll.type === "C") {
		...
		if (roll.text.match(/HR\d+/i) !== null) highRoll = parseInt(roll.text.substring(2));
		else highRoll = false;
		if (roll.text.match(/LR\d+/i) !== null) lowRoll = parseInt(roll.text.substring(2));
		else lowRoll = false;
