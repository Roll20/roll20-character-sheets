...
const validateWeaponSkills = (ids?: [string]) => {
  ...
  const prefixes = (ids && ids.map((id) => `repeating_weapons_${id}`)) || [
    "repeating_weapons",
  ];
  getAttrs(
    ["system", ...prefixes.map((p) => `${p}_weapon_skill_bonus`)],
    (v) => {
      const revisedList = [
          "@{skill_exert}",
          "@{skill_punch}",
...
const validateShells = (e: EventInfo) => {
  ...
  if (
    e.sourceAttribute.slice(-6) === "active" &&
    e.sourceAttribute.slice(-15, -7) !== "affinity"
  ) {
    console.log("Changing");
    getSectionIDs("repeating_shells", (idArray) => {
      idArray = idArray.filter(
        (item) => item !== e.sourceAttribute.slice(17, -13)
      );
      const setting: { [key: string]: string } = {};
      idArray.forEach(
        (id) => (setting[`repeating_shells_${id}_shell_active`] = "0")
      );
      setAttrs(setting, {}, () => {
        calculateShellAttrs();
      });
