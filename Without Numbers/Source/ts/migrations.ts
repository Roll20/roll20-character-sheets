  ...
  const performUpgrade = (version: string) => {
    ...
    if (major === 2 && minor < 1) {
        ...
        getAttrs(sourceAttrs, (v) => {
          const setting = idArray.reduce((m: { [k: string]: string }, id) => {
            ...
            if (v[`repeating_weapons_${id}_weapon_shock_damage`] !== "0")
              m[`repeating_weapons_${id}_weapon_shock`] =
                "{{shock=[[@{weapon_shock_damage} + @{weapon_attribute_mod}[Attribute] + @{weapon_skill_to_damage}[Skill]]] ^{SHOCK_DAMAGE_AGAINST_AC_LEQ} @{weapon_shock_ac}!}}";
            if (
              v[`repeating_weapons_${id}_weapon_ammo`] &&
              v[`repeating_weapons_${id}_weapon_ammo`] !== "0"
            ) {
              m[`repeating_weapons_${id}_weapon_use_ammo`] =
                "{{ammo=[[0@{weapon_ammo} - (1 @{weapon_burst})]] / @{weapon_ammo|max}}}";
            }
            return m;
          }, {});
      ...
      getSectionIDs("repeating_ship-weapons", (idArray) => {
        getAttrs(
          idArray.map((id) => `repeating_ship-weapons_${id}_weapon_ammo_max`),
          (v) => {
            const setting = idArray.reduce(
              (m: { [key: string]: string }, id) => {
                if (
                  v[`repeating_ship-weapons_${id}_weapon_ammo_max`] &&
      ...
      getSectionIDs("repeating_npc-attacks", (idArray) => {
        getAttrs(
          idArray.map((id) => `repeating_npc-attacks_${id}_attack_burst`),
          (v) => {
            const setting = idArray.reduce(
              (m: { [key: string]: string }, id) => {
            ...
            const setting = idArray.reduce(
              (m: { [key: string]: string }, id) => {
                m[`repeating_gear_${id}_gear_status`] = (
                  v[`repeating_gear_${id}_gear_status`] || ""
                ).toUpperCase();
                return m;
              },
              {}
      ...
      getAttrs(
        ...
        (v) => {
          if (v.armor_ac) {
            ...
            fillRepeatingSectionFromData("armor", data, () =>
              upgradeFunction()
            );
      ...
      getSectionIDs("repeating_skills", (skillIDs) =>
        getSectionIDs("repeating_magic-skills", (magicIDs) => {
          getSectionIDs("repeating_psychic-skills", (psychicIDs) => {
            const sourceAttrs = [
              ...skillIDs.map((id) => `repeating_skills_${id}_skill_query`),
              ...magicIDs.map(
                (id) => `repeating_magic-skills_${id}_skill_query`
              ),
              ...psychicIDs.map(
                (id) => `repeating_psychic-skills_${id}_skill_query`
              ),
            ...
            getAttrs(sourceAttrs, (v) => {
              const setting = sourceAttrs.reduce(
                (m: { [key: string]: string }, attrName) => {
      ...
      getSectionIDs("repeating_drones", (idArray) => {
        calculateDroneAttack(
          idArray.map((id) => `repeating_drones_${id}`),
          () => upgradeFunction()
        );
      });
      ...
      getAttrs(
        ...
        (v) => {
          const strain_extra =
            (parseInt(v.strain) || 0) - (parseInt(v.strain_permanent) || 0);
          mySetAttrs(
            {
...
const upgradeFrom162 = () => {
  console.log(
    "Upgrading from versionless sheet (assumed to be fresh or v1.6.2)."
  );
  ...
  // Legacy migration
  getAttrs(
    [1, 2, 3, 4, 5, 6, 7, 8].map((i) => `psionics_mastered_${i}`),
    (v) => {
      ...
      for (let i = 1; i < 9; i++) {
        ...
        if (technique) {
          ...
          setting[`repeating_techniques_${newRowId}_technique_name`] =
            technique;
        }
      }
      setAttrs(setting);
  ...
  getAttrs(
    ...
    (v) => {
      const setting = Object.entries(attrConversionData).reduce(
        (m: { [key: string]: string }, [oldName, newName]) => {
          if (
            v[oldName] &&
            v[oldName] !== "" &&
            `${v[newName]}` !== `${v[oldName]}`
      ...
      ["one", "two", "three"].forEach((num) => {
        if (v[`skill_culture_${num}_value`])
          setting[`skill_culture_${num}_name`] = `Culture/${
            v[`skill_culture_${num}_value`]
          }`;
      });
      ...
      fillRepeatingSectionFromData("resources", customCounterData, () =>
        upgradeFunction()
      );
      ...
      if (
        v.setting_space_magic_enable === "on" &&
        v.homebrew_psionics_disable !== "on"
      )
  ...
  getSectionIDs("repeating_weapons", (idArray) => {
    const oldAttrs = [
      ...
      ...idArray.map(
        (id) => `repeating_weapons_${id}_ship_weapon_special_effects`
      ),
    ...
    getAttrs(oldAttrs, (v) => {
      const setting = idArray.reduce((m: { [key: string]: string }, id) => {
        if (v[`repeating_weapons_${id}_add_skill`] === "@{weapon_skill_bonus}")
          m[`repeating_weapons_${id}_weapon_skill_to_damage`] =
            "@{weapon_skill_bonus}";
        if (v[`repeating_weapons_${id}_weapon_shock`])
          m[`repeating_weapons_${id}_weapon_shock_damage`] =
            v[`repeating_weapons_${id}_weapon_shock`];
        const modValue = v[`repeating_weapons_${id}_attribute_mod`];
        switch (modValue) {
          case "@{dex_bonus}":
            m[`repeating_weapons_${id}_weapon_attribute_mod`] =
              "@{dexterity_mod}";
            break;
          case "@{con_bonus}":
            m[`repeating_weapons_${id}_weapon_attribute_mod`] =
      ...
      const data = idArray
        .filter((id) => v[`repeating_weapons_${id}_ship_weapon_name`])
        .map((id) => {
          ...
          if (v[`repeating_weapons_${id}_ship_weapon_ab`])
            row.weapon_attack_bonus =
              v[`repeating_weapons_${id}_ship_weapon_ab`];
          ...
          if (v[`repeating_weapons_${id}_ship_weapon_special_effects`])
            row.weapon_qualities =
              v[`repeating_weapons_${id}_ship_weapon_special_effects`];
          if (v[`repeating_weapons_${id}_ship_weapon_broken`] === "on")
            row.weapon_broken = "1";
          return row;
        });
      fillRepeatingSectionFromData("ship-weapons", data, () =>
        upgradeFunction()
      );
  ...
  getSectionIDs("repeating_skills", (idArray) => {
    const oldAttrs = [
      ...
      ...idArray.map(
        (id) => `repeating_skills_${id}_custom_skill_1_specialist`
      ),
      ...idArray.map(
        (id) => `repeating_skills_${id}_custom_skill_2_specialist`
      ),
    ];
    getAttrs(oldAttrs, (v) => {
      const data = idArray.reduce((m, id) => {
        [1, 2].forEach((i) => {
          if (v[`repeating_skills_${id}_custom_skill_${i}_name`]) {
            const skillLevel =
              typeof v[`repeating_skills_${id}_custom_skill_${i}_level`] ===
              "undefined"
                ? "-1"
                : v[`repeating_skills_${id}_custom_skill_${i}_level`];
            m.push({
              skill_name: v[`repeating_skills_${id}_custom_skill_${i}_name`],
              skill: skillLevel,
              skill_specialist:
                v[`repeating_skills_${id}_custom_skill_${i}_specialist`] ||
                "2d6",
            });
          }
        });
        ...
        if (v[`repeating_technique_${id}_technique`])
          m.push({
            ...
            technique_description:
              v[`repeating_technique_${id}_technique_description`] || "",
          });
        return m;
      }, []);
  ...
  getSectionIDs("repeating_cyberware", (idArray) => {
    getAttrs(
      idArray.map((id) => `repeating_cyberware_${id}_cyberware`),
      (v) => {
        const setting = idArray.reduce((m: { [key: string]: string }, id) => {
          if (v[`repeating_cyberware_${id}_cyberware`])
            m[`repeating_cyberware_${id}_cyberware_name`] =
              v[`repeating_cyberware_${id}_cyberware`];
          return m;
        }, {});
        setAttrs(setting, null, () => upgradeFunction());
      }
    ...
    getAttrs(oldAttrs, (v) => {
      const setting = idArray.reduce((m: { [key: string]: string }, id) => {
        if (v[`repeating_goals_${id}_misc_goal`])
          m[`repeating_goals_${id}_goal_name`] =
            v[`repeating_goals_${id}_misc_goal`];
        if (v[`repeating_goals_${id}_misc_goal_xp`])
          m[`repeating_goals_${id}_goal_xp`] =
            v[`repeating_goals_${id}_misc_goal_xp`];
        return m;
      }, {});
      setAttrs(setting, null, () => upgradeFunction());
    });
  ...
  getSectionIDs("repeating_languages", (idArray) => {
    getAttrs(
      idArray.map((id) => `repeating_languages_${id}_languages`),
      (v) => {
        const setting = idArray.reduce((m: { [key: string]: string }, id) => {
          if (v[`repeating_languages_${id}_languages`])
            m[`repeating_languages_${id}_language`] =
              v[`repeating_languages_${id}_languages`];
          return m;
        }, {});
        setAttrs(setting, null, () => upgradeFunction());
      }
  ...
  getSectionIDs("repeating_gear", (idArray) => {
    getAttrs(
      idArray.map((id) => `repeating_gear_${id}_gear_readied`),
      (v) => {
        const setting = idArray.reduce((m: { [key: string]: string }, id) => {
  ...
  getSectionIDs("repeating_defenses", (idArray) => {
    const oldAttrs = [
      ...
      ...idArray.map(
        (id) => `repeating_defenses_${id}_ship_defense_special_effects`
      ),
    ...
    getAttrs(oldAttrs, (v) => {
      const data = idArray.map((id) => {
        ...
        if (v[`repeating_defenses_${id}_ship_defense_special_effects`])
          row.defense_effect =
            v[`repeating_defenses_${id}_ship_defense_special_effects`];
        if (v[`repeating_defenses_${id}_ship_defense_broken`])
          row.defense_broken = "1";
        return row;
      });
      fillRepeatingSectionFromData("ship-defenses", data, () =>
        upgradeFunction()
      );
    });
  });
  ...
  getSectionIDs("repeating_fittings", (idArray) => {
    const oldAttrs = [
      ...
      ...idArray.map(
        (id) => `repeating_fittings_${id}_ship_fitting_special_effects`
      ),
    ...
    getAttrs(oldAttrs, (v) => {
      const data = idArray.map((id) => {
        ...
        if (v[`repeating_fittings_${id}_ship_fitting_special_effects`])
          row.fitting_effect =
            v[`repeating_fittings_${id}_ship_fitting_special_effects`];
        if (v[`repeating_fittings_${id}_ship_fitting_broken`])
          row.fitting_broken = "1";
        return row;
      });
      fillRepeatingSectionFromData("ship-fittings", data, () =>
        upgradeFunction()
      );
    });
  });
