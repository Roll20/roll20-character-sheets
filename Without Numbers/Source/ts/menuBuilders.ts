...
const skillToMacro = (v: { [x: string]: string }) => (skill: string) => {
  ...
  if (namedSkills.includes(skill))
    return [
      `@{skill_${skill}_name} (${sign(parseInt(v[`skill_${skill}`]))})`,
      `skill_${skill}`,
    ];
  else
    return [
      `^{${skill.toUpperCase()}} (${sign(parseInt(v[`skill_${skill}`]))})`,
      `skill_${skill}`,
    ];
};
const idToSkillMacro =
  (v: { [x: string]: string }, sName: string) => (id: any) => {
    const prefix = `repeating_${sName}_${id}_skill`;
    return [`${v[`${prefix}_name`]} (${sign(parseInt(v[prefix]))})`, prefix];
  };
const buildSkillMacro = (
...
const buildSkillMenu = () => {
  getSectionIDs("repeating_skills", (idArray) => {
    ...
    getAttrs(sourceAttrs, (v) => {
      const skillList = skills[v.system] || [];
      const hasSkills =
        skillList.filter((skill) => v[`skill_${skill}`] !== "-1").length ||
        idArray.filter((id) => v[`repeating_skills_${id}_skill`] !== "-1")
          .length;

      const macro_skills = `${[
        ...skillList
          .filter((skill) => v[`skill_${skill}`] !== "-1")
          .map(skillToMacro(v)),
        ...idArray
          .filter((id) => v[`repeating_skills_${id}_skill`] !== "-1")
          .map(idToSkillMacro(v, "skills")),
      ]
        .map(buildSkillMacro)
        .join(" ")}${
        v.setting_show_untrained_skills === "1"
          ? `${hasSkills ? "\n\n**^{UNTRAINED}**\n" : ""}${[
              ...skillList
                .filter((skill) => v[`skill_${skill}`] === "-1")
                .map(skillToMacro(v)),
  ...
  getSectionIDs("repeating_techniques", (techniqueIDs) => {
    getSectionIDs("repeating_psychic-skills", (skillIDs) => {
      const sourceAttrs = [
        ...
        ...techniqueIDs.map(
          (id) => `repeating_techniques_${id}_technique_name`
        ),
      ];
      getAttrs(sourceAttrs, (v) => {
        if (
          v.setting_super_type === "magic" ||
          v.setting_super_type === "neither"
        )
          return;
        const macro_psionics = `${[
          ...skills.psionic
            ...
            .map(skillToMacro(v)),
          ...skillIDs
            .filter((id) => v[`repeating_psychic-skills_${id}_skill`] !== "-1")
          ...
          .map((id, index, allIDs) =>
            buildLink(
              v[`repeating_techniques_${id}_technique_name`],
              `repeating_techniques_${id}_technique`,
...
const buildMagicMenu = () => {
  getSectionIDs("repeating_spells", (spellIDs) => {
    getSectionIDs("repeating_magic-skills", (skillIDs) => {
      ...
      getAttrs(sourceAttrs, (v) => {
        if (
          v.setting_super_type === "psionics" ||
          v.setting_super_type === "neither"
        )
          return;
        const macro_magic = `${[
          ...(v.skill_magic2_name
            ? ["know_magic", "use_magic", "fight", "sunblade", "magic2"]
            : ["know_magic", "use_magic", "fight", "sunblade"]
          ).map(skillToMacro(v)),
          ...
          .map((id) =>
            buildLink(
              v[`repeating_spells_${id}_spell_name`],
              `repeating_spells_${id}_spell`
            )
          )
          .join(" ")}`;
        mySetAttrs(
...
const buildShipWeaponsMenu = () => {
  getSectionIDs("repeating_ship-weapons", (idArray) => {
    const sourceAttrs = [
      ...
      ...idArray.map(
        (id) => `repeating_ship-weapons_${id}_weapon_attack_bonus`
      ),
      "macro_ship_weapons",
    ];
    getAttrs(sourceAttrs, (v) => {
      const macro_ship_weapons = idArray
        .map((id, index) => {
          const title =
            v[`repeating_ship-weapons_${id}_weapon_name`] +
            ` ${sign(v[`repeating_ship-weapons_${id}_weapon_attack_bonus`])}`;
          return buildLink(
            title,
            `repeating_ship-weapons_${id}_attack`,
            index + 1 === idArray.length
          );
        })
        .join(" ");
      mySetAttrs(
...
const buildAttacksMenu = () => {
  getSectionIDs("repeating_npc-attacks", (idArray) => {
    ...
    getAttrs(sourceAttrs, (v) => {
      const macro_npc_attacks = idArray
        .map((id, index) => {
          const title =
            v[`repeating_npc-attacks_${id}_attack_name`] +
          ...
          return buildLink(
            title,
            `repeating_npc-attacks_${id}_attack`,
            index + 1 === idArray.length
          );
        })
        .join(" ");
...
const buildAbilitiesMenu = () => {
  getSectionIDs("repeating_npc-abilities", (idArray) => {
    ...
    getAttrs(sourceAttrs, (v) => {
      const macro_npc_abilities = idArray
        .map((id, index) => {
          return buildLink(
            v[`repeating_npc-abilities_${id}_ability_name`],
            `repeating_npc-abilities_${id}_ability`,
            index + 1 === idArray.length
          );
        })
        .join(" ");
