  ...
  getAttrs(
    ...
    (v) => {
      ...
      const setting: { [key: string]: any } = {
        save_physical:
          base -
          (Math.max(parseInt(v.strength_mod), parseInt(v.constitution_mod)) ||
            0),
        save_mental:
          base -
          (Math.max(parseInt(v.charisma_mod), parseInt(v.wisdom_mod)) || 0),
        save_evasion:
          base -
  ...
  getSectionIDs("repeating_psychic-skills", (idArray) => {
    getAttrs(
      [
        ...
        ...idArray.map((id) => `repeating_psychic-skills_${id}_skill`),
      ],
      (v) => {
        const attrBonus =
            Math.max(parseInt(v.wisdom_mod), parseInt(v.constitution_mod)) || 0,
          skillBonus = Math.max(
            ...skills.psionic.map((x) => parseInt(v[`skill_${x}`]) || 0),
            ...idArray.map(
              (id) => parseInt(v[`repeating_psychic-skills_${id}_skill`]) || 0
            )
          );
        const psionics_total_effort =
          Math.max(1 + attrBonus + skillBonus, 1) +
            parseInt(v.psionics_extra_effort) -
            parseInt(v.psionics_committed_effort_current) -
            parseInt(v.psionics_committed_effort_scene) -
...
const calculateMagicEffort = () => {
  getAttrs(
    [
      "magic_total_effort",
      "magic_committed_effort_current",
      "magic_committed_effort_scene",
      "magic_committed_effort_day",
      "magic_uncommitted_effort",
    ],
    (v) => {
      const magic_uncommitted_effort =
        parseInt(v.magic_total_effort) -
          (parseInt(v.magic_committed_effort_current) +
            parseInt(v.magic_committed_effort_scene) +
            parseInt(v.magic_committed_effort_day)) || 0;
      mySetAttrs(
        {
          magic_uncommitted_effort,
        },
...
const calculateProcessing = () => {
  getSectionIDs("repeating_processing-nodes", (idArray) => {
    const sourceAttrs = [
      ...
      "intelligence_mod",
      "ai_extra_processing",
      "ai_committed_processing_current",
      "ai_committed_processing_scene",
      "ai_committed_processing_day",
      "ai_total_processing",
    ];
    getAttrs(sourceAttrs, (v) => {
      const maxProcessing =
        (1 + Math.max(parseInt(v.wisdom_mod), parseInt(v.intelligence_mod)) ||
          0) +
        Math.max(
          ...idArray
            .filter(
              (id) =>
                v[`repeating_processing-nodes_${id}_node_connected`] === "1"
            )
            .map(
              (id) =>
                parseInt(v[`repeating_processing-nodes_${id}_node_value`]) || 0
            ),
          0
        );
      const ai_total_processing =
        maxProcessing +
          parseInt(v.ai_extra_processing) -
          parseInt(v.ai_committed_processing_current) -
          parseInt(v.ai_committed_processing_scene) -
          parseInt(v.ai_committed_processing_day) || 0;
      mySetAttrs(
        {
          ai_total_processing,
        },
...
const calculatePermanentStrain = () => {
  getAttrs(
    ["cyberware_strain_total", "strain_permanent_extra", "strain_permanent"],
    (v) => {
      const permStrain =
        parseInt(v.cyberware_strain_total) +
          parseInt(v.strain_permanent_extra) || 0;
      mySetAttrs(
        {
...
const calculateStrain = () => {
  ...
  getAttrs(
    ...
    (v) => {
      const strain =
        (parseInt(v.strain_permanent) || 0) + (parseInt(v.strain_extra) || 0);
...
const calculateShellAttrs = () => {
  ...
  getSectionIDs("repeating_shells", (idArray) => {
    const sourceAttrs = [
      ...
      ...physicalAttrs,
      "setting_transhuman_enable",
      "setting_ai_enable",
    ];
    getAttrs(sourceAttrs, (v) => {
      if (
        v.setting_transhuman_enable === "transhuman" ||
        v.setting_ai_enable === "ai"
      ) {
        let attributes: { [key: string]: string } = {};
        physicalAttrs.forEach(
          (attr) =>
            (attributes[attr] =
              idArray
                .filter(
                  (id) => v[`repeating_shells_${id}_shell_active`] === "1"
                )
        ...
        mySetAttrs(attributes, v, null, () => {
          physicalAttrs
            .filter((attr) => attributes[attr] !== "None")
            .forEach((attr) => calculateMod(attr));
        });
      } else {
...
const calculateGearReadiedStowed = () => {
  const doCalc = (
    ...
    getAttrs(attrs, (v) => {
      const [gear_readied, gear_stowed] = armorIDs.reduce(
        (m, id) => {
        ...
        weaponIDs.reduce(
          (m, id) => {
            if (v[`repeating_weapons_${id}_weapon_status`] === "READIED")
              m[0] +=
                parseInt(v[`repeating_weapons_${id}_weapon_encumbrance`]) || 0;
          ...
          gearIDs.reduce(
            (m, id) => {
              const amount =
                parseInt(v[`repeating_gear_${id}_gear_amount`]) || 0;
              let packingFactor = 1;
              ...
              if (v[`repeating_gear_${id}_gear_status`] === "READIED")
                m[0] +=
                  Math.ceil(
                    (amount *
                      parseFloat(v[`repeating_gear_${id}_gear_encumbrance`])) /
                      packingFactor
                  ) || 0;
      ...
      const armor_encumbrance_bonus = Math.max(
        0,
        ...armorIDs
          .filter((id) => v[`repeating_armor_${id}_armor_status`] === "READIED")
          .map(
            (id) =>
              parseInt(v[`repeating_armor_${id}_armor_encumbrance_bonus`]) || 0
          )
      );
      const gear_stowed_max =
        parseInt(v.strength) + armor_encumbrance_bonus || 0;
      ...
      const setting = {
        ...
        gear_stowed_max,
        gear_readied_over:
          parseInt(v.gear_readied) > gear_readied_max ? "1" : "0",
  ...
  getSectionIDs("repeating_gear", (gearIDs) => {
    getSectionIDs("repeating_weapons", (weaponIDs) => {
      getSectionIDs("repeating_armor", (armorIDs) =>
        doCalc(gearIDs, weaponIDs, armorIDs)
      );
    });
  });
};
  ...
  getSectionIDs("repeating_weapons", (idArray) => {
    ...
    getAttrs(sourceAttrs, (v) => {
      ...
      prefixes.forEach((prefix) => {
        const attrBonus =
          parseInt(
            v[(v[`${prefix}_weapon_attribute_mod`] || "").slice(2, -1)]
          ) || 0;
        const skillBonus =
          parseInt(v[(v[`${prefix}_weapon_skill_bonus`] || "").slice(2, -1)]) ||
          parseInt(v[`${prefix}_weapon_skill_bonus`]) ||
          0;
        const damageBonus =
          attrBonus +
          (v[`${prefix}_weapon_skill_to_damage`] === "@{weapon_skill_bonus}"
            ? skillBonus
            : 0);
        const weaponDamage =
          v[`${prefix}_weapon_damage`] === "0"
            ? ""
            : v[`${prefix}_weapon_damage`];
        const shockString =
          v[`${prefix}_weapon_shock`] !== "0"
            ? `, ${
                (parseInt(v[`${prefix}_weapon_shock_damage`]) || 0) +
                damageBonus
              }\xa0${translate("SHOCK").toString().toLowerCase()}${
                v[`${prefix}_weapon_shock_ac`]
                  ? ` ${translate("VS_AC_LEQ")} ${
                      v[`${prefix}_weapon_shock_ac`]
                    }`
                  : ""
        ...
        const attack =
          baseAttackBonus +
          (parseInt(v[`${prefix}_weapon_attack`]) || 0) +
          (skillBonus === -1 ? -2 : skillBonus) +
          attrBonus;
        const damage =
          weaponDamage +
          (weaponDamage
        ...
        setting[`${prefix}_weapon_attack_display`] =
          attack >= 0 ? `+${attack}` : attack.toString();
        setting[`${prefix}_weapon_damage_display`] = `${
          damage || 0
        }\xa0${translate("DAMAGE").toString().toLowerCase()}${shockString}`;
      });
      setting.macro_weapons = prefixes
        .map((prefix, index) => {
          const label = `${v[`${prefix}_weapon_name`]} (${
            setting[`${prefix}_weapon_attack_display`]
          })`;
          return buildLink(
            label,
      ...
      (v) => {
        const setting = idArray.reduce((m: { [k: string]: string }, id) => {
          m[`repeating_${sName}_${id}_weapon_api`] =
            v.setting_use_ammo === "1" &&
            v[`repeating_${sName}_${id}_weapon_use_ammo`] !== "0"
