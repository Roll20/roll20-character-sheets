...
attributes.forEach((attr) =>
  on(`change:${attr} change:${attr}_bonus`, () => calculateMod(attr))
);
...
on(
  "change:setting_transhuman_enable change:setting_ai_enable",
  calculateShellAttrs
);
...
on(
  "change:repeating_cyberware remove:repeating_cyberware",
  calculateCyberwareStrain
);
on(
  "change:strain_permanent_extra change:cyberware_strain_total",
  calculatePermanentStrain
);

on("change:level", calculateSaves);

on(
  [...effortAttributes, "repeating_psychic-skills:skill"]
    .map((x) => `change:${x}`)
    .join(" "),
  calculateEffort
);
...
on(
  "change:magic_committed_effort_current change:magic_committed_effort_scene change:magic_committed_effort_day change:magic_total_effort",
  calculateMagicEffort
);

on(
  "change:ai_committed_processing_current change:ai_committed_processing_scene change:ai_committed_processing_day change:ai_extra_processing change:repeating_processing-nodes remove:repeating:processing_nodes",
  calculateProcessing
);

on(
  "change:repeating_armor change:innate_ac remove:repeating_armor",
  calculateAC
);

on(
  "change:strength change:repeating_gear remove:repeating_gear change:repeating_weapons " +
    "remove:repeating_weapons change:repeating_armor remove:repeating_armor",
  calculateGearReadiedStowed
);
...
on(
  ...
  () => calculateDroneAttack(["repeating_drones"])
);
...
on(
  "change:npc_roll_full_attack change:repeating_npc-attacks:attack_number",
  setNPCMultiAttacks
);

on(
  "change:repeating_npc-abilities:ability_name remove:repeating_npc-abilities",
  buildAbilitiesMenu
);
on(
  "change:repeating_npc-attacks:attack_name change:repeating_npc-attacks:attack_ab " +
    "change:repeating_npc-attacks:attack_number remove:repeating_npc-attacks",
  buildAttacksMenu
);
on(
  "change:npc change:npc_armor_type change:macro_npc_attacks change:macro_npc_abilities",
  buildStatblock
);
on("change:npc_hd", handleNPCSave);
