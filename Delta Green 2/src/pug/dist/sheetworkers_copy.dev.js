...
arrays['_stats'].forEach(function (stat) {
  ...
  on("ready sheet:opened change:".concat(stat_score), function (eventinfo) {
    var value = parseInt(eventinfo.newValue) || 0;

    if (value < 0) {
      setAttributes(_defineProperty({}, stat_score, 0), false);
    }
  });
});
arrays['_skills'].forEach(function (attr) {
  on("ready sheet:opened change:".concat(attr), function (eventinfo) {
...
var version_170_200 = function version_170_200() {
  ...
  var update = {};
  update['version'] = codeversion;
  getAttrs(["motivations"], function (v) {
...
on("sheet:opened change:willpower_points change:sanity_points", function () {
