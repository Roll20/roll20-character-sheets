...
var _rd12 = "[[1d12]]";
var _rd20 = "[[1d20]]";
var queryModifier = "?{Modifier|,0|+20%,20|+40%,40|-20%,-20|-40%,-40|custom (%),?{custom (%)}}";
var prefix_roll = "@{gm_toggle} &{template:fancy-rolls} {{name=@{character_name}}} {{dice=[[".concat(_rd100, "]]}}");
var criticals = [1, 11, 22, 33, 44, 55, 66, 77, 88, 99, 100]; //levelup character
...
var rollwithmodifiers = function rollwithmodifiers(rollString, roll) {
  ...
  on("clicked:".concat(roll, "-action"), function (eventInfo) {
    ...
    if (arrays["_editable_skills"].includes(_roll)) {
      ...
      getAttrs(["".concat(_roll, "_name")], function (values) {
        ...
        var rollString = "".concat(prefix_roll, " ").concat(_header, " {{subheader=@{").concat(roll, "}}} ");
        rollwithmodifiers(rollString, roll);
      });
    } else {
...
var check_for_wp_modifiers = function check_for_wp_modifiers(values) {
  console.log('wp: ' + values['willpower_points']);
  console.log('low wp: ' + values['low_will_power']);
  console.log('zero wp: ' + values['zero_will_power']);
...
var clicked_repeating_skills = function clicked_repeating_skills(parameters, names) {
  getAttrs(parameters, function (values) {
    ...
    var rollString = "".concat(prefix_roll, " {{header=").concat(skillname, "}} {{subheader=").concat(skillrank, "}} "); ///////////////////////////////////////////////////////////
...
Object.entries(_repeating_sections).forEach(function (_ref5) {
  ...
  on("clicked:repeating_".concat(section, ":").concat(element, "-action"), function (eventInfo) {
    ...
    if (section === 'skills') {
      ...
      clicked_repeating_skills(_parameters, _input_names); //bond dependent parameters
