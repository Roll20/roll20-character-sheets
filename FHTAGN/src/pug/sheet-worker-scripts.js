...
on('sheet:opened', e => {
  getAttrs(['waffe-unbewaffnet-fert-wert', 'Waffenloser_Kampf-wert', 'Waffenloser_Kampf-basiswert'], values => {
    if (!values['waffe-unbewaffnet-fert-wert']) {
      setAttrs({
        'waffe-unbewaffnet-fert-wert': values['Waffenloser_Kampf-wert'] || values['Waffenloser_Kampf-basiswert'],
      })
    }
  })
});
  ...
  on(`change:${skillnameClean}-wert`, (e) => {
    ...
    getAttrs([...weaponTypes, ...weaponSkillValues, basiswertKey], (values) => {
      weaponKeys.forEach((key) => {
        ...
        const isUsingSkillValue = parseInt(currentWeaponValue) === parseInt(e.previousValue);
        const isBothEmpty = currentWeaponValue === '' && typeof e.previousValue === 'undefined';
        const isUsingBaseValue = parseInt(currentWeaponValue) === parseInt(values[basiswertKey]);
        const isMatcheable = isWeaponOfType && (isUsingSkillValue || isBothEmpty || isUsingBaseValue);
...
[0, 1, 2, 3, "unbewaffnet"].forEach((key) => {
  on(`change:waffe-${key}-schaden`, (e) => {
    const templateObject = {
      rollname: key === "unbewaffnet" ? "Unbewaffneter Schaden" : `Schaden mit @{waffe-${key}-name}`,
