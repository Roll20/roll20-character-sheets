...
let menuMap = {
  ...
  ex: {
    npcMacroName: 'NPC-ex',
    npcName: ' ^{npc} ',
    name: 'extraordinary-abilities-menu',
    section: 'ability',
    filterField: 'ability_type',
    filterValue: 'Ex',
    groupBy: 'frequency',
    translateGroup: 1,
    altUsesField: 'rounds_between',
    npcLinkField: 'npc-roll',
    useshowinmenu: 1,
  },
  sp: {
    npcMacroName: 'NPC-sp',
    npcName: ' ^{npc} ',
    name: 'spell-like-abilities-menu',
    section: 'ability',
    filterField: 'ability_type',
    filterValue: 'Sp',
    groupBy: 'frequency',
  ...
  su: {
    npcMacroName: 'NPC-su',
    npcName: ' ^{npc} ',
    name: 'supernatural-abilities-menu',
    section: 'ability',
    filterField: 'ability_type',
    filterValue: 'Su',
    groupBy: 'frequency',
  ...
  attacks: {
    ...
    altGroupBy: 'group',
    translateGroup: 1,
    npcLinkField: 'attack-npc-roll',
    linkField: 'attack-roll',
  },
};
...
export function getRepeatingCommandMacro(baseAttribs, callback, header) {
  let done = function (macro, origMacro) {
      if (typeof callback === 'function') {
        callback(macro, origMacro);
      }
    },
    ...
    filterValue = '',
    baseMacro =
      '&{template:REPLACETEMPLATE} @{toggle_attack_accessible} @{toggle_rounded_flag} {{font=@{apply_specfont_chat}@{use_specfont}}} {{font=@{apply_specfont_chat}@{use_specfont}}} {{scroll_desc=@{scroll-desc}}} {{color=@{rolltemplate_color}}} {{header_image=@{REPLACEHEADER}}} {{character_name=@{character_name}}} {{character_id=@{character_id}}} {{subtitle=REPLACESUBTITLE}} {{name=REPLACENPC^{REPLACENAME}}}',
        ...
        restOfMacro = _.chain(orderedList)
          .map(function (id) {
            ...
            try {
              ...
              if (bonusField && v[linePrefix + bonusField] && !(v[linePrefix + bonusField] === 'not-applicable' || v[linePrefix + bonusField] == '0')) {
                //double equal 0
...
export function resetOneCommandMacro(menuName, isNPC, callback, header, groupMap) {
  ...
  try {
    ...
    getRepeatingCommandMacro(
      params,
      function (newMacro, oldMacro) {
        let setter = {};
...
function updateAllMenusMenu(eventInfo) {
  getAttrs(['allmenus_macro', 'is_npc', 'unchained_skills-show', 'BG-Skill-Use'], function (v) {
    ...
    let skillsMacro = '{{row07=[^{combat-skills}](~@{character_id}|combat_skills) }} {{row08=[^{all-skills}](~@{character_id}|skills) }} ';
    ...
    if (!isNPC) {
      if (unchained && bgSkillUse === 1) {
        skillsMacro = '{{row07=[^{adventure}](~@{character_id}|adventure_skills) }} {{row08=[^{background}](~@{character_id}|background_skills) }} ';
      ...
      if (unchained && bgSkillUse === 1) {
        skillsMacroNPC = '{{row07=[^{adventure}](~@{character_id}|NPC-adventure_skills) }} {{row08=[^{background}](~@{character_id}|NPC-background_skills) }} ';
