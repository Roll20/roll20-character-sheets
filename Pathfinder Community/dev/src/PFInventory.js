...
var locationNamesOld = ['Belt', 'Body', 'Chest', 'Eyes', 'Feet', 'Hands', 'Head', 'Headband', 'Neck', 'Ring1', 'Ring2', 'Shoulders', 'Wrist'],
  wornSlotTypes = locationNamesOld.concat(['Armor3', 'Shield3', 'Armor', 'Shield']), // added Armor and Shield to help match
  wornSlots = _.map(wornSlotTypes, function (row) {
    return 'worn-' + row;
  }),
  wornSlotsRolls = _.map(wornSlotTypes, function (row) {
        ...
        getSectionIDs('repeating_item', function (ids) {
          ...
          getAttrs(attribList, function (v) {
            ...
            try {
              ...
              matchingField = _.find(fields, function (field) {
              ...
              if (_.size(newRowAttrs) > 0) {
                ...
                SWUtils.setWrapper(newRowAttrs, PFConst.silentParams, function () {
                  if (_.size(locationAttrs) > 0) {
                    SWUtils.setWrapper(locationAttrs, {}, function () {
                      done(isWorn);
                    });
                  } else {
                    done(isWorn);
  ...
  getAttrs(['migrated_worn_equipment'], function (v) {
    var foundWornArmor = false,
      ...
      doneWornRows = _.after(_.size(locationNamesOld), function () {
        ...
        try {
          wornRows = _.map(locationNamesOld, function (field) {
            return 'worn-' + field + '-';
          });
    ...
    getAttrs([locationField, oldLocationField, nameField, locationTabField], function (v) {
      ...
      try {
        ...
        if (location === locationMap.Carried && oldlocation !== locationMap.NotCarried && oldlocation !== location) {
          ...
          if (oldlocation === 2) {
            // Armor un-equipped, reset Defenses
            setter['armor3-equipped'] = 0;
          }
          if (oldlocation === 14) {
            // Shield un-equipped, reset Defenses
            setter['shield3-equipped'] = 0;
          }
        } else if (location > locationMap.NotCarried) {
...
function updateWornArmorAndShield(location, sourceAttribute, callback) {
  ...
  try {
    ...
    attribList = _.reduce(
      PFDefense.defenseArmorShieldColumns,
      function (memo, col) {
  ...
  getAttrs(attribList, function (w) {
    var i = 0,
      ...
      itemName = '';
    try {
      //if we are setting new, or updating an item in the location, or updating an item in a different location
      ...
      if (attribUpdated === 'set-as-armor' || attribUpdated === 'set-as-shield' || location === locationMap.Armor || location === locationMap.Shield) {
        ...
        for (i = 0; i < PFDefense.defenseArmorShieldColumns.length; i++) {
          if (
            PFDefense.defenseArmorShieldColumns[i] !== 'max-dex' &&
        ...
        //this would only be if "set as armor or shield" checked
        if (
          (location === locationMap.Armor && equipType === equipMap.Armor && actualLocation !== locationMap.Armor) ||
          (location === locationMap.Shield && equipType === equipMap.Armor && actualLocation !== locationMap.Shield)
        ) {
          silentSetter[item_entry + 'old_location'] = actualLocation;
          silentSetter[item_entry + 'location'] = location; // should set location to match set armor or shield button
  ...
  getSectionIDs('repeating_item', function (ids) {
    ...
    getAttrs(wornSlotsRolls, function (v) {
      var notFoundList = [],
        ...
        setter = {};
      try {
        lowerIDs = _.map(ids, function (id) {
        ...
        notFoundList = _.chain(v)
          ...
          .omit(function (val, key) {
            return _.contains(lowerIDs, val);
          })
          .value();
...
function deleteWornRow(source, location, type, name, callback) {
  const searchSource = source.toLowerCase();
  getAttrs(['armor3', 'shield3', ...wornSlotsRolls], function (v) {
    var match = '',
      row = '',
      attrib = '',
      armorName = v.armor3 || '',
      shieldName = v.shield3 || '',
      setter = {};
    try {
      match = _.findKey(v, function (val) {
        ...
        if (val.toLowerCase().indexOf(searchSource) >= 0) {
          // worn item
          return true;
        }
        return false;
      ...
      if (match) {
        // Case 1: Slotted item found (worn slot, or as armor3/shield3 roll)
        row = match.slice(0, -5);
        setter[match] = '';
        setter[row] = ''; // Clears the row ID or item name

        // If the matched slot was for Armor or Shield, clear Defenses
        if (location === 2) {
          attrib = 'armor3';
        } else if (location === 14) {
          attrib = 'shield3';
        ...
        // Case 2: NO slotted item found, check Defenses for match vs non-slotted equipment
        if (type === 2 && name === armorName) {
          attrib = 'armor3';
        } else if (type === 2 && name === shieldName) {
          attrib = 'shield3';
        }
      }
      ...
      if (attrib) {
        setter[attrib] = '';
        setter[attrib + '-equipped'] = 1;
        setter[attrib + '-acbonus'] = 0;
        setter[attrib + '-enhance'] = 0;
        setter[attrib + '-max-dex'] = '-';
        setter[attrib + '-acp'] = 0;
        setter[attrib + '-spell-fail'] = 0;
        setter[attrib + '-type'] = '';
        setter[attrib + '-proficiency'] = 1;
      }

      // Only call setWrapper if the setter object has attributes to update.
      if (_.size(setter) > 0) {
        SWUtils.setWrapper(setter, PFConst.silent, callback);
      } else if (typeof callback === 'function') {
        // If no attributes were changed, call the callback immediately.
        callback();
      }
    } catch (err) {
      TAS.error('PFInventory.event delete item for attribute: ' + source, err);
      if (typeof callback === 'function') {
        // Call callback even on error to prevent execution from hanging
        callback();
      }
    }
  });
...
export function migrate(callback, oldversion) {
  ...
  PFMigrate.migrateRepeatingItemAttributes(
    TAS.callback(function callPFInventorySetNewDefaults() {
      setNewDefaults(
        TAS.callback(function callPFInventoryMigrateWornEquipment() {
...
function registerEventHandlers() {
  var tempstr = '';
  on(
    'change:repeating_item:item-category_compendium',
    TAS.callback(function EventItemCompendium(eventInfo) {
  ...
  on(
    'change:repeating_item:location',
    TAS.callback(function eventUpdateItemLocation(eventInfo) {
      // TAS.debug('caught ' + eventInfo.sourceAttribute + ' event: ' + eventInfo.sourceType);
      if (eventInfo.sourceType === 'player' || eventInfo.sourceType === 'api') {
  ...
  on(
    'change:repeating_item:qty_max',
    TAS.callback(function eventUpdateItemMaxQty(eventInfo) {
      if (eventInfo.sourceType === 'player' || eventInfo.sourceType === 'api') {
        TAS.debug('caught ' + eventInfo.sourceAttribute + ' event: ' + eventInfo.sourceType);
  ...
  on(
    'change:repeating_item:qty',
    TAS.callback(function eventUpdateItemTotalQty(eventInfo) {
      if (eventInfo.sourceType === 'player' || eventInfo.sourceType === 'api') {
        TAS.debug('caught ' + eventInfo.sourceAttribute + ' event: ' + eventInfo.sourceType);
        getAttrs(['repeating_item_qty_max'], function (v) {
  ...
  on(
    'change:repeating_item:item-weight',
  ...
  on(
    ...
    TAS.callback(function eventRemoveItem(eventInfo) {
      ...
      // Find the correct key ie exclude 'old_location'
      const locationAttr = Object.keys(eventInfo.removedInfo).find((key) => key.endsWith('_location') && key.length > '_old_location'.length);
      const typeAttr = Object.keys(eventInfo.removedInfo).find((key) => key.endsWith('_equip-type'));
      const nameAttr = Object.keys(eventInfo.removedInfo).find((key) => key.endsWith('_name'));
      location = locationAttr ? +eventInfo.removedInfo[locationAttr] : null;
      type = typeAttr ? +eventInfo.removedInfo[typeAttr] : null;
      name = nameAttr ? eventInfo.removedInfo[nameAttr] : '';
      // type and name can be sent to deleteWornRow to check if the deleted row was
      // armor or shield that was not worn and also clear it from Defenses
      if (eventInfo.sourceType === 'player' || eventInfo.sourceType === 'api') {
        console.log(`Change detected: Removed Row location:${location} type:${type} name:${name}`);
        updateRepeatingItems();
        deleteWornRow(eventInfo.sourceAttribute, location, type, name);
      }
      PFAttacks.removeLinkedAttack(null, PFAttacks.linkedAttackType.equipment, SWUtils.getRowId(eventInfo.sourceAttribute));
    }),
  );
  on(
    'change:CP change:SP change:GP change:PP',
  ...
  on(
    'change:carried-currency change:carried-currency-toggle change:item_total_weight change:carried-misc',
  ...
  on(
    'change:repeating_item:item-defense-type change:repeating_item:item-acbonus change:repeating_item:item-max-dex change:repeating_item:item-acp change:repeating_item:item-spell-fail change:repeating_item:item-proficiency change:repeating_item:acenhance',
    TAS.callback(function eventUpdateEquippedArmorOrShield(eventInfo) {
      var location = 0;
      TAS.debug('caught ' + eventInfo.sourceAttribute + ' event' + eventInfo.sourceType);
      getAttrs(['repeating_item_location'], function (v) {
  ...
  _.each(commonLinkedAttributes, function (fieldToWatch) {
    ...
    on(
      eventToWatch,
      TAS.callback(function eventupdateAssociatedAttackLoop(eventInfo) {
        TAS.debug('caught ' + eventInfo.sourceAttribute + ' event' + eventInfo.sourceType);
        if (eventInfo.sourceType === 'player' || eventInfo.sourceType === 'api') {
  ...
  on(
    'change:repeating_item:name change:repeating_item:item-dmg-type change:repeating_item:item-proficiency change:repeating_item:default_size change:repeating_item:wpenhance',
  ...
  on(
    'change:repeating_item:create-attack-entry',
    TAS.callback(function eventcreateAttackEntryFromRow(eventInfo) {
      TAS.debug('caught ' + eventInfo.sourceAttribute + ' event: ' + eventInfo.sourceType);
      if (eventInfo.sourceType === 'player' || eventInfo.sourceType === 'api') {
  ...
  on(
    'change:repeating_item:set-as-armor',
    TAS.callback(function eventcreateArmorEntryFromRow(eventInfo) {
      TAS.debug('caught ' + eventInfo.sourceAttribute + ' event: ' + eventInfo.sourceType);
      if (eventInfo.sourceType === 'player' || eventInfo.sourceType === 'api') {
  ...
  on(
    'change:repeating_item:set-as-shield',
    TAS.callback(function eventcreateShieldEntryFromRow(eventInfo) {
      TAS.debug('caught ' + eventInfo.sourceAttribute + ' event: ' + eventInfo.sourceType);
      if (eventInfo.sourceType === 'player' || eventInfo.sourceType === 'api') {
  ...
  on(
    'change:repeating_item:showinmenu change:repeating_item:equip-type change:repeating_item:name',
    TAS.callback(function eventShowItemInMenu(eventInfo) {
      TAS.debug('caught ' + eventInfo.sourceAttribute + ' event: ' + eventInfo.sourceType);
      if (eventInfo.sourceType === 'player' || eventInfo.sourceType === 'api') {
        resetCommandMacro();
      }
  ...
  on(
    'change:repeating_item:name',
    TAS.callback(function eventUpdateWornItemName(eventInfo) {
      TAS.debug('caught ' + eventInfo.sourceAttribute + ' event: ' + eventInfo.sourceType);
      if (eventInfo.sourceType === 'player' || eventInfo.sourceType === 'api') {
        updateEquipmentLocation(null, null, null, eventInfo);
      }
  ...
  on(
    'change:repeating_item:equip-type',
    TAS.callback(function eventItemEquipTypeChange(eventInfo) {
      TAS.debug('caught ' + eventInfo.sourceAttribute + ' event: ' + eventInfo.sourceType);
      if (eventInfo.sourceType === 'player' || eventInfo.sourceType === 'api') {
